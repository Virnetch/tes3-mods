...
> _Adds NPC services for recharging enchanted items, deciphering magic scrolls and the option to transcribe magic scrolls, both by the player, and through a service. Adds empty scrolls from OAAB to enchanters and booksellers, and disables the passive recharging of enchanted items. Highly customizable through the Mod Config Menu._
...
- Fixed Blank Scrolls item addition and related errors (Update MWSE!)
- Fixed Right Click Menu Exit compatibility

#### 0.9.5

- Fixed merchants not being able to barter with the gold they receive from the services.

#### 0.9.4

- Fixed Azura's Star (and Azura's Black Star from Necrocraft) getting consumed when transcribing.

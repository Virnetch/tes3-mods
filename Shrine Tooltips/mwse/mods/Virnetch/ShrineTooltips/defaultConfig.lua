return {
	modEnabled = true,
	detailedTooltip = false,
	logLevel = "INFO",

	-- All shrine scripts
	shrineScripts = {
		["shrineAralor"] = true,
		["shrineDelyn"] = true,
		["shrineFelms"] = true,
		["shrineLlothis"] = true,
		["shrineMeris"] = true,
		["shrineNerevar"] = true,
		["shrineOlms"] = true,
		["shrineRilm"] = true,
		["shrineRoris"] = true,
		["shrineSeryn"] = true,
		["shrineVeloth"] = true,
		["shrineVivecFury"] = true,
		["shrineTemple"] = true,

		["shrineImperial"] = true,

			--Tamriel_Data https://www.nexusmods.com/morrowind/mods/44537
		["T_ScObj_ShrineAlessia"] = true,
		["T_ScObj_ShrineAlmalexiaMercy"] = true,
		["T_ScObj_ShrineCuhlecain"] = true,
		["T_ScObj_ShrineMorihaus"] = true,
		["T_ScObj_ShrineReman"] = true,
		["T_ScObj_ShrineSothaSilMastery"] = true,

		["T_ScObj_ShrineSaintColoPC"] = true,
		["T_ScObj_ShrineSaintEmpPC"] = true,
		["T_ScObj_ShrineSaintHealerPC"] = true,
		["T_ScObj_ShrineSaintHearthPC"] = true,
		["T_ScObj_ShrineSaintLawPC"] = true,
		["T_ScObj_ShrineSaintProphPC"] = true,
		["T_ScObj_ShrineSaintTradePC"] = true,
		["T_ScObj_ShrineSaintWarPC"] = true,
		["T_ScObj_ShrineSaintWorkPC"] = true,

		["T_ScObj_ShrinePelinal"] = true,
		["T_ScObj_ShrineOrdinator"] = true,

			-- OAAB_Data https://www.nexusmods.com/morrowind/mods/49042
		["AB_ShrineAlma_s"] = true,
		["AB_ShrineSotha_s"] = true,

			-- mtrDaedricOfferings https://www.nexusmods.com/morrowind/mods/49697
		["mtrShrineAzura"] = true,
		["mtrShrineBoethiah"] = true,
		["mtrShrineMalacath"] = true,
		["mtrShrineMehrunesDagon"] = true,
		["mtrShrineMolagBal"] = true,
		["mtrShrineSheogorath"] = true,

			-- Solstheim - AllMaker's Blessings https://www.nexusmods.com/morrowind/mods/50416
		["shrineAllMaker"] = true
	},

	-- All blessing spell ids
	blessingIds = {
		["aralor's intervention"] = true,
		["shield of st. delyn"] = true,
		["felms' glory"] = true,
		["the rock of llothis"] = true,
		["meris' warding"] = true,
		["veloth's indwelling"] = true,
		["spirit of nerevar"] = true,
		["olms' benediction"] = true,
		["rilm's grace"] = true,
		["roris' bloom"] = true,
		["seryn's shield"] = true,
		["vivec's fury"] = true,
		["lady's grace shrine"] = true,
		["soul of sotha sil"] = true,
		["vivec's mystery"] = true,

			--Tamriel_Data https://www.nexusmods.com/morrowind/mods/44537
		["T_Imp_Res_BlessingAlessia"] = true,
		["T_De_Res_AlmalexiasMercy"] = true,
		["T_Imp_Res_BlessingCuhlecain"] = true,
		["T_Imp_Res_BlessingMorihaus"] = true,
		["T_Imp_Res_BlessingReman"] = true,
		["T_De_Res_SothaSilsMastery"] = true,

		["T_Imp_Res_BlessingStColo"] = true,
		["T_Imp_Res_BlessingStEmp"] = true,
		["T_Imp_Res_BlessingStHealer"] = true,
		["T_Imp_Res_BlessingStHearth"] = true,
		["T_Imp_Res_BlessingStLaw"] = true,
		["T_Imp_Res_BlessingStProph"] = true,
		["T_Imp_Res_BlessingStTrade"] = true,
		["T_Imp_Res_BlessingStWar"] = true,
		["T_Imp_Res_BlessingStWork"] = true,

		["T_Imp_Res_BlessingPelinal"] = true,
		["T_De_Res_OrdinatorBlessing"] = true,

			-- mtrDaedricOfferings https://www.nexusmods.com/morrowind/mods/49697
		["mtrFavorAzura"] = true,
		["mtrFavorBoethiah"] = true,
		["mtrFavorMalacath"] = true,
		["mtrFavorMehrunesDagon"] = true,
		["mtrFavorMolagBal"] = true,
		["mtrFavorSheogorath"] = true,

			-- Solstheim - AllMaker's Blessings https://www.nexusmods.com/morrowind/mods/50416
		["hmc_bm_ab_endurethecold"] = true,
		["hmc_bm_ab_traversetheland"] = true
	}
}